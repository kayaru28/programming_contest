...
int main(){
    ...
    rep(bi,bitval){
        ...
        for( ll index = keta-1 ; index >=0 ; index-- ){
            if(xbit[index]==1){
                ls.push_back( getllSubstr(S,index,1));
            }else{
                rs.push_back( getllSubstr(S,index,1));
            }
        }
        ...
        for(ll tmp : ls) li = (li*10) + tmp;
        for(ll tmp : rs) ri = (ri*10) + tmp;
