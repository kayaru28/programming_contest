...
int main(){
    ...
    getSameGcdPairs(R,L);
    /*
    for( ll gcdvar = 2 ; gcdvar <=R ; gcdvar++ ){
        cout << same_gcd_pairs[gcdvar] << endl;
    }
    cout << "---------------------" << endl;
    */

    for( ll gcdvar = 2 ; gcdvar <=R ; gcdvar++ ){
        ll cnt_baisu = same_gcd_pairs[gcdvar];
        ll cnt_pair = cnt_baisu * 2;//(cnt_baisu-1);
        ans += cnt_pair;
    }
    //cout << ans << endl;

    for( ll pvar = llmax(2,L) ; pvar <=R ; pvar++ ){
        ll cnt_baisu = R/pvar;
        cnt_baisu--;
        ll cnt_pair = cnt_baisu * 2;
        ans -= cnt_pair;
    }
    cout << ans << endl;
